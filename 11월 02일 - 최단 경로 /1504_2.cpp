//
// 1504번 - 특정한 최단 경로
// 다익스트라 버전 -> 다익스트라를 시작지점을 다르게 하여 3번수행해 주면됨
...
vector<int> dijkstra(int vertex, int start, vector<vector<ci>> &graph) {
    ...
    priority_queue<ci, vector<ci>, greater<>> pq; //first : 시작점으로부터 vertex까지 거리, second : vertex

    //시작 위치 초기화
    dist[start] = 0;
    pq.push({0, start}); // 시작점부터 시작점까지 거리 0, 시작점
    ...
    while (!pq.empty()) {
        ...
        if (weight > dist[node]) //이미 확인했던 정점
            continue;
        for (int i = 0; i < graph[node].size(); i++) { //해당 정점에 연결된 모든 정점에 대하여
 ...
 *    조건에 의하면 E(간선의 수)의 값이 0일 수도 있음. 이 경우에선 1->1->N->N의 값이 INF이라 경로가 없음에도 3*INF 조건에 걸리지 않음.
...
int main() {
    ...
    vector<vector<int>> dist(3, vector<int>(n + 1, 0));// 1, v1, v2각각을 시작점으로 하는 다익스트라 결과
    while (e--) { //무방향 그래프
        cin >> a >> b >> c;
