...
vector<string> topologicalSort(set<string> &item) {
    ...
    priority_queue<string, vector<string>, greater<>> pq, pq_temp;//알파벳 순으로 정렬(우선순위 큐 사용)

    for (auto iter = item.begin(); iter != item.end(); iter++) {//보유하고 있는 모든 아이템에 대하여
        cout << *iter << "\n";
    ...
    while (!pq.empty()) {
        ...
        for (int i = 0; i < graph[node].size(); i++) {//node에 연결된 다른 노드들을 하나씩 탐색
        ...
        if (pq.empty() && !pq_temp.empty()) //이전에 진입차수가 0이였던 정점을 모두 탐색했고, 새로 탐색할 정점이 있다면
            swap(pq, pq_temp);//같은 회차에 진입차수가 0인 것 끼리 정렬을 해야하므로 tmp에 다음회차에 0이되는 것들을 모아두고 마지막에 반영
    }
    return result;
}
