...
bool bfs(vector<string> &place, int row, int col) {//방의 정보, 응시자의 위치 (x,y)
    int dr[4] = {-1, 1, 0, 0};
    int dc[4] = {0, 0, -1, 1};

    vector<vector<int>> dist(SIZE, vector<int>(SIZE, 0)); //방문 여부, 각 정점까지의 맨허튼 거리(응시자의 위치로부터 해당 지점까지의 맨해튼 거리)
